...
474: 1, 625: 1, 577: 1, 414: 1, 19: 1}
    ...
    for line in file_handle:
        ...
        if value not in number_freqs:
            number_freqs[value] = 1
        else:
            number_freqs[value] += 1
...
def invert(dictionary:dict[str,int])->dict[int,list[int]]:
  '''
  inverts a dictionary of integers to become a a dictionary of
  int to a sorted list of int

  >>> invert(get_number_frequencies('numbers_small.txt'))
  {2: [865], 5: [200, 771], 1: [633, 866]}
  '''
  inverted = {}
...
def most_freq_number(filename: str) -> list[int]:
    ...
    [455, 927]
    """
    frequencies = get_number_frequencies(filename)
    freq_vals = invert(frequencies)
    cur_highest = 0
...
37: ['Liechtenstein']})
    ...
    for line in file_handle:
        line = line.strip()
        info = line.split(',')
        name_population[info[0]] = int(info[1])

        pop_bin = int(info[1])//granularity
        if pop_bin not in granularity_name:
            granularity_name[pop_bin] = [info[0]]
        else:
            granularity_name[pop_bin].append(info[0])

    tup_info = (name_population,granularity_name)
    return tup_info
...
(2876101, 'Albania'), (2881355, 'Jamaica'), (2924816, 'Armenia')]
    ...
    tup_info = create_population_dictionaries(filename, granularity)
    pop_info = []

    if country in tup_info[0]:

        bin = tup_info[0][country]//granularity
        country_names = tup_info[1][bin]
        for place in country_names:
            tup = (tup_info[0][place],place)
            pop_info.append(tup)
        pop_info.sort()
